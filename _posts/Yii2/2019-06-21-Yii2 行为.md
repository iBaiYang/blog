...
extends继承关系：
common\models\LeaveType >> yii\db\ActiveRecord >> yii\db\BaseActiveRecord >> yii\base\Model >> yii\bas\Component >> yii\bas\Object 
...
insertInternal()也在yii\db\ActiveRecord中：
```
...
这里可以看到调用了事件处理程序trigger()，trigger()写在yii\bas\Component中：
```
...
ensureBehaviors()及其调用的attachBehaviorInternal()也都在yii\bas\Component中：
```
public function ensureBehaviors()
{
    if ($this->_behaviors === null) {
...
可以看到，上面这里把写在LeaveType中的behaviors()填充到Component->_behaviors[]中，为Behavior行为事件做准备：
