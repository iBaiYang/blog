...
![]({{site.baseurl}}/images/20190823/20190823154944.png)

![]({{site.baseurl}}/images/20190823/20190823155037.png)
...
保存在`yii\base\Component`中的`private $_events`的数据结构：
```
...
保存在`yii\base\Componen`t中的`private $_events`的数据结构：
```
$_events[Person::EVENT_GREET] = [
    [
        'person_say_hello',
...
现在Event中的`private static $_events`储存的数据结构：
```
...
此时Component中的`private $_events`的数据结构是：
```
$_events['bar'][] = [
    [
        function ($event) {                     
