...
`yii\log\Logger`对象的`$dispatcher`变量又赋值为`log`组件，同时`log`日志组件的`flushInterval`参数调用`setFlushInterval()`赋值时，
也会把`yii\log\Logger`对象的`$flushInterval`变量赋值过去，这也就是上面`yii\log\Logger`对象的`$flushInterval`的来源。
