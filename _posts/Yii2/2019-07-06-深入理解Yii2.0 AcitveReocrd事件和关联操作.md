...
可以把这3个操作两两组合作为 transactions() 所返回数组元素的值。 如，self::OP_INSERT|self::OP_UPDATE 表示插入和更新操作。 也可以直接使用OP_ALL 表示3种操作都包含。
...
在上面 insert() 的代码中，通过我们的声明，Yii发现需要事务支持， 于是就调用了static::getDb()->beginTransaction() 来启用事务。 
