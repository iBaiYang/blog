...
代码可以看出，Service Locator继承自 yii\base\Component ，这是Yii中的一个基础类， 提供了属性、事件、行为等基本功能。
...
其中， $_components 用于缓存存Service Locator中的组件或服务的实例。 Service Locator 为其提供了getter和setter。
...
这也是为了往 $_definitions 中写服务组件，不然怎么把属性和组件区分开呢。
...
所以Application其实就是一个Service Locator。
