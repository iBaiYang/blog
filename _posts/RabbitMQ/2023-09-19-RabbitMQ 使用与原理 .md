...
3、修改3台RabbitMq服务器的nostname。3台服务器的nostname分别设置为node1、node2、node3：`vi /etc/hostname`。
...
6、启动RabbitMQ服务，顺带启动Erlang虚拟机和RbbitMQ应用服务，分别在3个节点上执行以下命令：`rabbitmg-server -detached`

7、在节点2执行

`rabbitmgctl stop app` (rabbitmqctl stop_会将Erlang虚拟机关闭，rabbitmgctl stop_app只会关闭rabbitmq.服务)

`rabbitmgctl reset` (重置)

`rabbitmgctl join cluster rabbit@node1` (节点2加入节点1)
...
rabbitmqctl start_app
rabbitmqctl cluster_status
rabbitmgctl forget cluster node rabbit@node2 (node1节点上执行)
```
...
客户端通过Haproxy代理服务器获得站点页面，而代理服务器收到客户请求后根据负载均衡的规则将请求数据转发给后端真实服务器
* 实现了一种事件驱动、单一进程模型，能支持非常大的并发连接数

生产者和消费者都连接HAProxy，HAProxy下面 node （node1 -> node2 -> node3），其中一个节点宕机后，其他节点还可以正常服务。
...
启动后，在浏览器中访问：http://192.168.16.138:8100/rabbitmq stats
