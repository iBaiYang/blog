...
class Geohash
{
    ...
    {
        // 维度一直运算到精度以下，如 $plat = 0.1，则 $latbits = 9
        ...
        // 经度一直运算到精度以下，如 $plong = 0.1，则 $longbits = 10
        ...
        $longbits = $bits;
        $latbits = $bits;
        $addlong = 1;  // $addlong 在 1 和 0 间 变动，第一轮经度精度加1，第二轮维度精度加1，以此类推
        while (($longbits + $latbits) % 5 != 0) {  // 直到经纬度精度之和被5整除，32位中以5位为1个单位，如 1 为 00001
            $longbits += $addlong;
            $latbits += !$addlong;
            $addlong = !$addlong;
        }

        $blat = $this->binEncode($lat, -90, 90, $latbits);  // 获取维度二进制字符串
        $blong = $this->binEncode($long, -180, 180, $longbits);  // 获取经度二进制字符串
    ...
    {
        $err = ($max - $min) / 2;  // 最大值减最小值的平均值，如 $min = -90，$max = 90，则 $err = 90
