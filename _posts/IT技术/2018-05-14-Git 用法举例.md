...
再一次用composer更新 phpoffice/phpspreadsheet 软件包时，发现这个包依赖的包 psr/simple-cache 和 markbaker/complex ，git添加时丢失了，
发现无论怎么改.gitignore文件都无法添加文件到版本控制，在 /vendor/psr/simple-cache 文件夹内git add时报了错：
Pathspec '/vendor/psr/simple-cache' is in submodule '/vendor/psr/simple-cache'，搜索后找到方案，要删除git缓存，然后重新添加：
```
