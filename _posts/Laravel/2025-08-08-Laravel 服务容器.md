...
$app = new Illuminate\Foundation\Application(
    // __DIR__ 表示脚本执行的所在目录的绝对路径，即 index.php 所在目录，
    // 是 public 目录 所在绝对路径，不到末尾 / 或者 \ 。
    // 再次 dirname(__DIR__)  后提升到项目根目录下
...
看下继承的父类的`static::setInstance($this)`方法：
```php
/**
 ...
 * 
 * $basePath 为 $_ENV['APP_BASE_PATH'] ?? dirname(__DIR__) 。
 * __DIR__ 是一个魔术常量，表示当前执行脚本所在的绝对目录路径，如：
 * /var/www/html/laravel-project/public
 * dirname(__DIR__) 提升到项目根目录下，如：
 ...
 * 
 * 如：
 * /var/www/html/laravel-project/app
 *
 * @param  string  $path
 * @return string
 */
...
几乎所有的 服务容器 绑定都会在 **服务提供者** 中注册，下面示例中的大多数将演示如何在该上下文（服务提供者）中使用容器。
例外：如果某个容器不依赖于任何接口就没必要去绑定类到这个容器里。容器不需要指定如何构建这些对象，因为它可以使用反射来自动解析这些对象（如每个控制器的请求方法中的 Illuminate\Http\Request 参数）。

在服务提供者中，可以通过 `$this->app` 属性访问容器。
...
扩展绑定：
```php
// extend 方法可以修改解析的服务。例如，当一个服务被解析后，你可以添加额外的代码去修饰或配置这个服务。 
// extend 方法接受一个闭包，闭包的唯一参数和返回值都是一个服务
...
例如下面最常用的在控制器的构造函数中定义 Request类 作为依赖：
```php
<?php
...
Laravel 服务容器会处理以下两方面的工作：
1. 注册基础服务；
2. 管理所需创建的类及其依赖。

### 注册基础服务提供者

分析完上面`registerBaseBindings()`后 ，再看下接下来的注册基础服务提供者：
```php
/**
...
1. 执行register 方法，将服务提供者提供的服务绑定到服务容器中 `$provider->register()`;
...
对于 Laravel 服务容器来讲，其内部实现上无论是 bind、singleton、tag 还是 extend 它们的基本原理大致类似。所以这里仅研究 bind 绑定来管中窥豹。
...
protected function resolve($abstract, $parameters = [], $raiseEvents = true)
{
    ...
    // 否则也就是说我们的服务还存在依赖，然后不断的去解析嵌套的依赖，直到它们可以去构建（isBuildable）。
...
bootstrap/cache/services.php 文件实例：
...
<?php return array (
  ...
  array (
    ...
    31 => 'App\\Providers\\BailianServiceProvider',
    32 => 'App\\Providers\\ElasticsearchServiceProvider',
  ),
  'eager' => 
  array (
    0 => 'Illuminate\\Auth\\AuthServiceProvider',
    ...
    18 => 'App\\Providers\\BailianServiceProvider',
    19 => 'App\\Providers\\ElasticsearchServiceProvider',
  ),
  'deferred' => 
  array (
...
什么是依赖注入 <https://learnku.com/articles/10738/what-is-dependency-injection>

深入浅出依赖注入 <https://learnku.com/articles/10928/deep-inflow-of-dependency-injection>

是否需要使用依赖注入容器 <https://learnku.com/articles/10773/do-you-need-to-use-a-dependency-injection-container>
