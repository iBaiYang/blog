...
应用场景1：有时候版本的一些优化需求开发到一半，可能其中某一个开发完的需求要临时上，
或者某些原因导致待开发的需求卡住了已开发完成的需求上线。这时候就需要把 commit 抽出来，单独处理。

应用场景2：有时候开发分支中的代码记录被污染了，导致开发分支合到线上分支有问题，这时就需要拉一条干净的开发分支，
再从旧的开发分支中，把 commit 复制到新分支。
...
![]({{site.baseurl}}/images/20221013/20221013152416.jpg)

需要把 b 复制到另一个分支，首先把 commitHash 复制下来，然后切到 master 分支。

![]({{site.baseurl}}/images/20221013/20221013152418.jpg)

当前 master 最新的记录是 a，使用cherry-pick把 b 应用到当前分支。

![]({{site.baseurl}}/images/20221013/20221013152420.jpg)
...
![]({{site.baseurl}}/images/20221013/20221013152422.jpg)

还是 feature 分支，现在需要把 c、d、e 都复制到 master 分支上。先把起点c和终点e的 commitHash 记下来。

![]({{site.baseurl}}/images/20221013/20221013152424.jpg)

切到 master 分支，使用区间的cherry-pick。可以看到 c 被成功复制，当进行到 d 时，发现代码冲突，cherry-pick中断了。
这时需要解决代码冲突，重新提交到暂存区。

![]({{site.baseurl}}/images/20221013/20221013152426.jpg)

然后使用```cherry-pick --continue```让cherry-pick继续进行下去。最后 e 也被复制进来，整个流程就完成了。
...
应用场景：有一天测试突然跟你说，你开发上线的功能有问题，需要马上撤回，否则会影响到系统使用。
这时可能会想到用 reset 回退，可是你看了看分支上最新的提交还有其他同事的代码，用 reset 会把这部分代码也撤回了。
由于情况紧急，又想不到好方法，还是任性的使用 reset，然后再让同事把他的代码合一遍（同事听到想打人），
于是你的技术形象在同事眼里一落千丈。
...
![]({{site.baseurl}}/images/20221013/20221013152428.jpg)

> git revert 21dcd937fe555f58841b17466a99118deb489212

revert 掉自己提交的 commit。

![]({{site.baseurl}}/images/20221013/20221013152430.jpg)

因为 revert 会生成一条新的提交记录，这时会让你编辑提交信息，编辑完后 `:wq` 保存退出就好了。

![]({{site.baseurl}}/images/20221013/20221013152432.jpg)
...
![]({{site.baseurl}}/images/20221013/20221013152434.jpg)

现在的 master 分支里多了条合并提交。

![]({{site.baseurl}}/images/20221013/20221013152436.jpg)

使用刚刚同样的 revert 方法，会发现命令行报错了。

为什么会这样？在官方文档中有解释。

通常无法 revert 合并，因为您不知道合并的哪一侧应被视为主线。
此选项指定主线的父编号（从1开始），并允许 revert 反转相对于指定父编号的更改

我的理解是因为合并提交是两条分支的交集节点，而 git 不知道需要撤销的哪一条分支，
需要添加参数 -m 指定主线分支，保留主线分支的代码，另一条则被撤销。

-m 后面要跟一个 parent number 标识出"主线"，一般使用 1 保留主分支代码。
> git revert -m 1 <commitHash>

3、revert 合并提交后，再次合并分支会失效

还是上面的场景，在 master 分支 revert 合并提交后，然后切到 feature 分支修复好 bug，
再合并到 master 分支时，会发现之前被 revert 的修改内容没有重新合并进来。

因为使用 revert 后， feature 分支的 commit 还是会保留在 master 分支的记录中，当你再次合并进去时，
git 判断有相同的 commitHash，就忽略了相关 commit 修改的内容。

这时就需要 revert 掉之前 revert 的合并提交，有点拗口，接下来看操作吧。

![]({{site.baseurl}}/images/20221013/20221013152440.jpg)

现在 master 的记录是这样的。

![]({{site.baseurl}}/images/20221013/20221013152442.jpg)
...
应用场景：某天你眼花，发现自己在其他人分支提交了代码还推到远程分支，这时因为分支只有你的最新提交，
就想着使用```reset --hard```，结果紧张不小心记错了 commitHash，reset 过头，把同事的 commit 搞没了。
没办法，```reset --hard```是强制回退的，找不到 commitHash 了，只能让同事从本地分支再推一次（同事瞬间拳头就硬了，怎么又是你）。
于是，你的技术形象又一落千丈。

**命令使用**

![]({{site.baseurl}}/images/20221013/20221013152446.jpg)

分支记录如上，想要 reset 到 b。

![]({{site.baseurl}}/images/20221013/20221013152448.jpg)

误操作 reset 过头，b 没了，最新的只剩下 a。

![]({{site.baseurl}}/images/20221013/20221013152450.jpg)

这时用git reflog查看历史记录，把错误提交的那次 commitHash 记下。

![]({{site.baseurl}}/images/20221013/20221013152452.jpg)
