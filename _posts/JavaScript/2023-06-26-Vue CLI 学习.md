...
2、ES标准规范意外，微软比较特别：TypeScript,是JS的一个超集。
typescript,添加了ES规范之外的很多微软自己创造的特性。
...
Angular:特点：]av3程序员开发
特点：
1、将后台的MC模式搬到了前端开发的框架当中，使前端框架具备了模块化的概念
...
React:
特点：
【虚拟D0M】，减少真实的D0M操作。有效提高了前端页面渲染效率。
缺点：使用复杂。额外学习一门语言：JS语高。
...
向这个语言添加了可选的静态类型和基于类的面向对象编程。由安德斯·海尔斯伯格(C#、Delphi、.
TypeScript之父；.NET创立者)主导。
...
在MVWM架构中，是不允许数据和视图直接通信的，只能通过ViewModel来通信，而ViewModel就是定义了一个Observer观察者。
...
这个骨架项目就是脚手架；可以参考我以前写的LeeSite项目骨架生成工具)，用于快速生成一个ue的项目模板。
