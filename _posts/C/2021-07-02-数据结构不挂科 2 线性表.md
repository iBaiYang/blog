...
* 例题2-4  假设利用两个线性表LA和LB分别表示两个集合A和B，现要求一个新的集合`A = A U B`。这就要求对线性表作如下操作： 
  扩⼤线性表LA，将存在于线性表LB中而不存在于线性表LA中的数据元素插入到LA中去。
  只要从线性表LB中依次取得每个数据元素，并依值在线性表LA中进⾏查访，若不存在，则插入之。
...
* 例题2-7  将顺序表 (a1，a2，... ，an) 重新排列为以 a1 为界的两部分：a1 前面的值均⽐ a1 ⼩，
  a1 后面的值都⽐ a1 ⼤(这里假设数据元素的类型具有可⽐性, 不妨设为整型)。 这一操作称为划分。a1 也称为基准。
...
A.插入、删除操作更方便 B.可以进⾏随机访问 C.可以省略表头指针或者表尾指针 D.访问前后相邻结点更灵活
...
（1）当前数据元素 ai ⽐ a1 ⼤时，表明它已经在 a1 的后面，不必改变它与 a1 之间的位置，继续⽐较下一个。
