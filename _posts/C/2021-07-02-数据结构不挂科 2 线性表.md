...
链表是⼀个动态的结构，它不需要提前分配空间，因此⽣成链表的过程是⼀个结点“逐个插⼊” 的过程。
...
(2) 将此结点的数据域赋值为e，并将它的 next 指针指向第⼀个结点，即L。
...
删除链表中的某个元素e，如果e在链表中出现不⽌⼀次，将删除第⼀次出现的e，否则什么也不做。 ⽤p找到元素e所在的结点：
...
头插法建⽴起来的链表的实际顺序与输⼊顺序刚好相反，输出时为倒序。
...
头插法与尾插法相反，建⽴起来的链表的实际顺序与输⼊顺序相同，输出时为顺序。
...
p=q到作⽤是只要有新结点插⼊，那么就会变成尾部结点，然后不断循环此操作。
...
因为下⾯的printf输出的是当前位置的下⼀个结点的数据，若写成`head != NULL`，
则指针指到NULL前个结点时，`(head->next)->data`⽆法输出。
...
最后⼀个结点的指针域的指针⼜指回第⼀个结点的链表。

和单链表的差别仅在于，判别链表中最后⼀个结点的条件不再是“后继是否为空”，⽽是“后继是否为头结点”。

* 例题2-10

在设尾指针的单循环链表上实现将两个线性表`(a1，a2，a3，…an)`和`(b1，b2，b3，…bn)`链接成⼀个线性表的运算。
...
* 例题2-11  与单链表相⽐。双链表的优点之⼀是（ ）。
  
A.插⼊、删除操作更⽅便 B.可以进⾏随机访问 C.可以省略表头指针或者表尾指针 D.访问前后相邻结点更灵活

#### 静态链表

借助数组来描述线性表的链式存储结构，这⾥的指针是结点的相对地址（数组下标）。
与顺序表⼀样，静态链表需要预先分配⼀块连续的存储空间。
...
| | 顺序表 | 链表 |
| :----: | :----: | :----: |
| 存取⽅式 | 顺序存取/随机存取 | 从表头顺序存取 |
| 逻辑结构与物理结构 | 逻辑相邻即物理相邻 | 逻辑相邻物理不⼀定相邻 |
| 存储密度 | =1 | <1 |
| 空间分配 | 预先分配 | 需要时申请分配 |

* 例题2-12  若线性表最常⽤的操作是存取第i个元素及其前驱和后继元素的值，为了提⾼效率，应采⽤（ ）的存储⽅式
...
第⼆，不论链表是否为空，其头指针是指向头结点的⾮空指针，链表的头指针不变，因此空表和⾮空表的处理统⼀了。

* 解析2-9

算法思路： 依次取原链表中的每个结点，将其作为第⼀个结点插⼊到新链表中去，指针p⽤来指向当前结点，p为空时结束。
...
该算法只是对链表中顺序扫描⼀边即完成了倒置，所以时间性能为O(n)。
...
在双链表中可以快速访问任何⼀个结点的前后相邻结点，⽽在单链表中只能快速访问任何⼀个结点的后继结点。
