...
在顺序队列中，当队尾指针已经到数组的上界，不不能再有入队操作，但其实数组中还有空位置，
...
队列（包括循环队列列）是一个逻辑概念，而链表是一个存储概念。一个队列列是否是循环队列，
不取决于它将采⽤何种存储结构，根据实际的需要，循环队列可以采用顺序存储结构，
...
A．元素的进出满⾜先进后出   B．元素的进出满⾜先进先出   C．只允许在端点进行插入和删除操作   D．⽆无共同点
...
本题考查堆栈和队列的特点。堆栈和队列是常用的两种数据结构，其中队列只允许在一端进⾏行插入，
另一端进⾏删除操作，具有先进先出的特征。⽽堆栈只允许在同一端进⾏插⼊和删除运算，具有后进先出的特征。
因此，堆栈和队列的相同之处是允许在端点进⾏插⼊和删除操作。

* 解析3-4

算法思想：将队列中的元素逐个地出队列，入栈：全部⼊栈后再逐个出栈，入队列。
...
线性表是具有相同特性的数据元素的一个有限序列。栈是限定仅在表尾进⾏插入和删除操作的线性表。
