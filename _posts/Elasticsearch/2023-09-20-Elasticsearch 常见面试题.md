...
Id应该用keyword.。keyword字段通常用于排痉，汇总和Term查询，例如term。
...
除了上述字段类型之外，其他类型都必须显示映射，也就是必须手工指定，因为其他类型S无法自动识别。
...
* _all搜索相当于在所有有索引的字段中检索：GET /product/_sexrch?q=2021-06-01
```
#验证a11搜索
PUT product
{
...
1. term和match phrasel区别：
* match phrase会将检索关键词分词，match phrasel的分词结果必须在被检索字段的分词中都包含，而且顺序必须相同，而且默认必须都是连续的
* term搜索不会将搜索词分词
2. term和keyword区别
* term是对于搜索词不分词，
