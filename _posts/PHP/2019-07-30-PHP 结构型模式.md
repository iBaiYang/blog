...
2. 代理模式 —— 代理类继承自被代理类，提供了与被代理类相同的接口，访问者与代理类进行交互。
3. 装饰器模式 —— 装饰器通过依赖注入，为注入对象添加一个或多个功能，装饰器提供了与被装饰类相同的方法。
