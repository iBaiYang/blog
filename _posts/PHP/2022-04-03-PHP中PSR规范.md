...
接口名称与 container-interop 中讨论的一致（只是为了符合其他 PSRs 而改变了命名空间）。接口名称是在 container-interop [4] 中彻底讨论，并投票决定的 [5] 。
...
接口需要包含那些方法是通过对现有的容器进行统计分析后得到的 [6] 。
...
容器中是否需要提供方法来定义对象，在 container-interop 项目开始时已经被讨论过了 [4] 。讨论的结果是接口不需要提供这个方法，因为它不在容器接口的目的中（查看「目的」部分）。
...
然而，一些已经实现的有其他可选参数的容器；这在技术上是合法的。这些容器也与 PSR-11 兼容 [11] 。
...
讨论的结果 [7] 是这超出了 $id 是用来从容器获取对象的范围， $id 是不知道对象是怎么创建的。对象参数更适合工厂类。
...
因此，如果用户捕获到了实现 Psr\Container\NotFoundExceptionInterface 接口的异常，可能意味着两种情况 [9] ：
