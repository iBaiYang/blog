...
为什么这里只输出一行呢？思考下。因为没有使用 foreach 或 while 循环往下进行，所以只输出了第一次的结果。
...
多线程和多进程都是操作系统参与的调度，而协程是用户自主实现的调度，协程的关键点实际上是“用户层实现自主调度”，
大概有“翻身农奴把歌唱”的意思。
...
上面的代码，运行以后，我们在等待curl发起请求的5秒钟内，同时可以完成文件写入功能，如果换做平时的PHP程序，
