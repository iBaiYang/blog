...
但是发现容器内composer安装好后，composer要使用git，容器内git的安装却不顺利。

进入容器后，到`/usr/local/src`目录下:
> cd /usr/local/src
...
现在我们可以使用了，切换到项目目录下执行：
> php /usr/local/src/composer.phar install/update ***

因为composer要使用git，我们在php容器中要安装好git，但容器内安装git安装不上。
