...
da9824e0ef9d6e7c12907d0694abaf43da95571b4fd121751d8d78d53c03f932
[root@localhost ~]#
[root@localhost ~]# docker ps -a
CONTAINER ID        IMAGE                     COMMAND                  CREATED             STATUS                             PORTS                                         NAMES
da9824e0ef9d        gitlab/gitlab-ce:latest   "/assets/wrapper"        23 seconds ago      Up 22 seconds (health: starting)   0.0.0.0:8000->80/tcp, 0.0.0.0:8001->443/tcp   gitlab
e8314bf2b462        registry                  "/entrypoint.sh /e..."   5 hours ago         Up 5 hours                         0.0.0.0:5000->5000/tcp                        registry
[root@localhost ~]#
```

现在浏览器访问 192.168.56.108:8000 可以看到 gitlab 的登录页面。

#### 重置密码

进入容器，重置 root 账户密码为 12345678 ：
```
